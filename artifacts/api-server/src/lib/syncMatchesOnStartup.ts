...
export async function syncMatchesOnStartup(): Promise<void> {
  ...
  if (!apiKey) {
    logger.warn("FOOTBALL_DATA_API_KEY não configurada — sincronização de jogos ignorada");
    return;
  }
  ...
  try {
    ...
    logger.info({ created, updated, total: fdMatches.length }, "Jogos sincronizados com sucesso");
  } catch (err) {
